...
class CNNparametrization(Parameterization):
    def __init__(self, model_type, model_path, stat_path=None):
        ...
        with open(stat_path) as json_file:
            ...
            # self.input_mean = torch.tensor(data['input_mean_train'])[0].reshape(4, 1, 1)
            # self.input_std = torch.tensor(data['input_std_train'])[0].reshape(4, 1, 1)
            # self.output_mean = torch.tensor(data['output_mean_train'])[0].reshape(2, 1, 1)
            # self.output_std = torch.tensor(data['output_std_train'])[0].reshape(2, 1, 1)
    ...
    def evaluate_model(self, model, input_tensor):
        ...
        # input_tensor = (input_tensor - input_tensor.mean(axis=(1,2))[:, None, None]) / (input_tensor.std(axis=(1,2))[:, None, None])
        
        # Test input stats: 
        # print("INPUT: ", input_tensor.shape)
        # print("MEAN: ", input_tensor.mean(axis=(1,2)))
        # print("STD: ", input_tensor.std(axis=(1,2)))
        # print("--------------------")
...
def run_simulation(pyqg_params, parameterization=None, sampling_freq=24*60*60.):
    ...
    # Set q1_q2
    # init_condition = xr.open_dataset('/media/volume/sdc/qg/TransferLearning-QG/init_eddy.nc')
    init_condition = xr.open_dataset('/media/volume/sdc/qg/TransferLearning-QG/init_case4.nc')
    print(init_condition['q'].isel(lev=0).shape)
    m.set_q1q2(
                init_condition['q'].isel(lev=0).values.astype(np.float64),
                init_condition['q'].isel(lev=1).values.astype(np.float64),
                )
...
base_kwargs = dict(tmax=50*year, dt=1*hr, twrite=10000, tavestart=5*year, taveint=1*day)
...
# }
sims['case4'] = {
    'label': 'spectral_filter_results',
    'cnn_path':'results/spectral_filter_results/BestModelBasedOnTestLoss.pt',
    'bnn_path':'results/BNN/BestModelBasedOnTestLoss.pt',
    ...
    'cnn_stat_path': 'results/spectral_filter_results/training_mean_std.txt',
    'bnn_stat_path': 'results/BNN/training_mean_std.txt',
...
# sims['bnn'] = {
#     'label': 'bnn_new_normalization',
#     'cnn_path':'results/spectral_filter_eddy/BestModelBasedOnTestLoss.pt',
#     'bnn_path':'results/BNN/BestModelBasedOnTestLoss.pt',
#     'tl_path':'results/BNN/final_model_after_training.pt',
#     'shallow_path':'results/BNN/BestModelBasedOnTestLoss.pt',

#     'cnn_stat_path': 'results/spectral_filter_eddy/training_mean_std.txt',
#     'bnn_stat_path': 'results/BNN/training_mean_std.txt',
...
for key, value in sims.items():
    ...
    # cnn.to_netcdf('eddy_40years_spectral_filter.nc')
    # print("cnn done")
